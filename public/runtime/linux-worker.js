...
(function (console) {
  ...
  const host_callbacks = {
    ...
    wasm_panic: (msg) => {
      ...
      port.postMessage({method:'log', message:'FATAL: ' + message});
  ...
  const message_callbacks = {
    init: (message) => {
      ...
      const wasm_error = (error) => {
        port.postMessage({method:'log',message:'FATAL: Wasm execution failed: '+error.toString()});
      ...
      const user_executable_setup = () => {
        ...
        user_executable_imports = {
          env: {
            ...
            browser_agent: (base, stack) => runBrowserAgent({
              base, stack, memory, mailbox: message.browser_mailbox,
              syscall: (number, ...args) => vmlinux_instance.exports['wasm_syscall_' + args.length](stack, 0, number, ...args),
