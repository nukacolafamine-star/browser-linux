...
import {patchKernelSpinlockReads} from './kernel-fix.mjs';
...
const patched=patchKernelSpinlockReads(stripped.bytes);
await WebAssembly.compile(patched.bytes);
await fs.writeFile(path.join(root,'public/assets/vmlinux.wasm'),patched.bytes);
console.log(`Kernel payload: ${originalKernel.length} → ${stripped.bytes.length} bytes (DWARF metadata removed)`);
const wabt=await wabtFactory();
console.log(`Kernel compatibility: ${patched.patch.changedLoads} shared-lock reads made atomic`);
