...
COPY patch-egl.py /tmp/patch-egl.py
RUN python3 /tmp/patch-egl.py

# Build Emscripten's SDL2 pthread port and expose its generated pkg-config file
# to QEMU's Meson dependency discovery. SDL runs in the canvas-owning pthread.
...
RUN mkdir -p /out/runtime/vendor /out/pack /out/provenance /out/sources && \
    ...
    cp /qemu/pc-bios/bios-256k.bin /qemu/pc-bios/vgabios-stdvga.bin /qemu/pc-bios/kvmvapic.bin /qemu/pc-bios/linuxboot_dma.bin /qemu/pc-bios/efi-virtio.rom /out/pack/ && \
    cp /tmp/browser-linux-egl-patch.json /out/provenance/ && \
