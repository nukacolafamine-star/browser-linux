...
/*
 * The launcher loads libcurl with dlopen(RTLD_LOCAL), so OpenSSL is not in
 * the global scope that RTLD_NEXT searches; look inside the loaded library.
 */
static void *real_symbol(const char *library, const char *name)
{
...
int X509_STORE_load_file(X509_STORE *store, const char *file)
{
    ...
    if (is_bundle(file)) {
        if (!load_path) load_path = (int (*)(X509_STORE *, const char *))real_symbol("libcrypto.so.3", "X509_STORE_load_path");
        if (load_path) return load_path(store, directory);
    }
    if (!real) real = (int (*)(X509_STORE *, const char *))real_symbol("libcrypto.so.3", "X509_STORE_load_file");
...
int SSL_CTX_load_verify_locations(SSL_CTX *ctx, const char *file, const char *path)
{
    ...
    if (!real) real = (int (*)(SSL_CTX *, const char *, const char *))real_symbol("libssl.so.3", "SSL_CTX_load_verify_locations");
    if (!real) return 0;
