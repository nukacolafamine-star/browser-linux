/* Browser Linux: Emscripten 4.0.10 errno -> Linux x86_64 9P2000.L.
 * Host constants come from Emscripten's musl errno.h; destination values from
 * musl's generic Linux errno.h. Preserve errors; never return success for an
 * unknown host error. See patch-9p-errno.py and probe-9p-errno.c.
 */
#ifndef BROWSER_LINUX_9P_ERRNO_EMSCRIPTEN_H
#define BROWSER_LINUX_9P_ERRNO_EMSCRIPTEN_H
#include <errno.h>
static inline int browser_linux_errno_to_dotl(int err)
{
    switch (err) {
    case 0: return 0;
    case EPERM: return 1;
    ...
    /* WASI values without distinct names in Emscripten's errno.h. */
    case __WASI_ERRNO_NOTSUP: return 95; /* Linux EOPNOTSUPP */
    case __WASI_ERRNO_NOTCAPABLE: return 1; /* Linux EPERM: capability denied */
    default: return 5; /* Linux EIO; unknown errno remains a failure. */
    }
}
#endif
